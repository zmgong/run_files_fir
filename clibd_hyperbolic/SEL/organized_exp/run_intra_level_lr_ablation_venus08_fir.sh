...
# Copy venv to node-local $SLURM_TMPDIR so Python startup (site module / site-packages) does not hang on slow shared $HOME
echo "Copying venv to \$SLURM_TMPDIR ..."
rsync -a ~/venvs/clibd-hyperbolic/ $SLURM_TMPDIR/venv_clibd/
