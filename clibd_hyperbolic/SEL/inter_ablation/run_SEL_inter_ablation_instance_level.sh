...
srun python scripts/train_cl.py 'model_config=for_bioscan_5m/hyperbolic/stacked_entailment_loss/inter_ablation/SEL_inter_ablation_instance_level.yaml' bioscan_5m_data.path_to_hdf5_data=$SLURM_TMPDIR/BIOSCAN_5M.hdf5 &
train_pid=$!
wait $train_pid
train_exit=$?
train_pid=""
...
if [ "$FINISHED" != "True" ]; then
  ...
  export PROJECT_ROOT
  sbatch --export=ALL,CHAIN_CHECKPOINT_DIR,PROJECT_ROOT "$(dirname "$0")/run_SEL_inter_ablation_instance_level.sh"
  exit 0
fi

echo "Training finished. Running inference, encode and cone check..."
srun python scripts/inference_and_eval.py 'model_config=for_bioscan_5m/hyperbolic/stacked_entailment_loss/inter_ablation/SEL_inter_ablation_instance_level.yaml' bioscan_5m_data.path_to_hdf5_data=$SLURM_TMPDIR/BIOSCAN_5M.hdf5 inference_and_eval_setting.eval_on=val
srun python scripts/encode_embeddings_to_parquet.py 'model_config=for_bioscan_5m/hyperbolic/stacked_entailment_loss/inter_ablation/SEL_inter_ablation_instance_level.yaml' bioscan_5m_data.path_to_hdf5_data=$SLURM_TMPDIR/BIOSCAN_5M.hdf5
srun python scripts/check_taxonomy_cone_statistics.py --parquet_path ./parquet_embeddings/bioscan_5m/SEL_inter_ablation_instance_level/val --output_dir ./cone_check_results/SEL_inter_ablation_instance_level
